...
public class CoreCommand implements CommandExecutor {
    ...
    private void handlePlayer(CommandSender commandSender, String[] args) {
       ...
       if(args.length == 0){
           ...
           for(GHCommand cmd : cmdManager.getRegisteredCommands()){
              if(p.hasPermission(cmd.getPermission())) p.sendMessage(ChatColor.AQUA+cmd.getCmd()+ ChatColor.GREEN+" - "+cmd.getDesc());
           }
    ...
    private void handleConsole(String[] args) {
        if(args.length == 0){
            ...
            for(GHCommand cmd : cmdManager.getRegisteredCommands()){
                pl.getLogger().info(ChatColor.AQUA+cmd.getCmd()+ ChatColor.GREEN+" - "+cmd.getDesc());
            }
